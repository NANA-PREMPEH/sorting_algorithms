#include "deck.h"
/**
 * aux_number_fun - turn into integer card value
 * @head_tmp1: pointer to the list
 * Return: integer rep
 **/
int aux_number_fun(deck_node_t *head_tmp1)
{
	int aux_number, j;
...
/**
 * number_sort - sorts a doubly linked list of integers, 4 stages
 * @list: pointer to the list head
 * Return: no return
 **/
void number_sort(deck_node_t **list)
{
	deck_node_t *head_tmp1, *head_tmp2, *aux1, *aux2;
	int flag = 0, i, aux_number1, aux_number2;
	unsigned int k;
	...
	{ k =  head_tmp1->card->kind;
		...
		{
			aux_number1 = aux_number_fun(head_tmp1);
			aux_number2 = aux_number_fun(head_tmp1->next);
			flag = 0;
			head_tmp2 = head_tmp1;
			while (head_tmp2 && head_tmp2->card->kind == k && aux_number1 > aux_number2)
			{
				aux1 = head_tmp2;
				aux2 = head_tmp2->next;
				aux1->next = aux2->next;
				...
				if (!head_tmp2)
					break;
				aux_number1 = aux_number_fun(head_tmp2);
				aux_number2 = aux_number_fun(head_tmp2->next);
			}
			if (flag == 0)
